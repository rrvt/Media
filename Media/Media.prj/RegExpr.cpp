...
bool RegExpression::match(TCchar* stg) {
...
Tchar* RegExpression::unanchoredMatch(TCchar* line, int index) {
...
Tchar* RegExpression::anchoredMatch(TCchar* line, int index) {
short    i;
short    rslt;
Pattern* p;
TCchar*  subStg;
...
short RegExpression::omatch(TCchar* s, Pattern* pat) {
Tchar ch = *s;
...
bool RegExpression::setWildCardPattern(TCchar* name) {
TCchar* p = name;
Tchar   re[512];
Tchar*  q = re;
...
bool RegExpression::setPattern(TCchar* pat) {
short i;
Tchar ch;
...
bool RegExpression::collectSet(TCchar** s, Pattern* p) {
TCchar* stg    = *s;
Tchar   ch;
Tchar   lastch = 0;
bool    hat    = false;
...
bool RegExpression::installKleene(short i) {
short  j = i-1;
PatKey key;
...
void RegExpression::clrSet(ulong set[]) {
short i;
...
void RegExpression::setBit(Tchar ch, ulong set[]) {set[setIndex(ch)] |= indexMask(ch);}


bool RegExpression::inSet(Tchar ch, ulong set[]) {return (set[setIndex(ch)] & indexMask(ch)) != 0;}


void RegExpression::invertSet(ulong set[]) {
short i;
